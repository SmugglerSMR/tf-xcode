...
public struct PostureNetV1: Layer {
    @noDerivative
    ...
    public init(classCount: Int = 2, alpha: Float = 1.0) {
        ...
        print("PostureNetV1:init   classCount: \(classCount)    alpha: \(alpha)")
        ...
        //output = Dense(inputSize: classCount, outputSize: 2, activation: hardSwish)
    }
    


    public mutating func readCheckpoint(to location: URL, name: String) {
        print("PostureNetV1:readCheckpoint: \(name)  URL: \(location.path)")
        ...
        do {
            ...
            print("PostureNetV1 loaded from checkpoint successfully.")
        } catch {
    ...
    public mutating func readPythonCheckpoint(to location: URL, name: String) {
        print("\nPostureNetV1:readPythonCheckpoint: \nURL: \(location.path)")
        ...
        do {
            ...
            print("PostureNetV1 loaded from python checkpoint  \(numberLayers) layers!\n")
        } catch {
            // If checkpoint is invalid, throw the error and exit.
            print("Fail to load PostureNetV1 from python checkpoint. \(error)")
        }
    ...
    public func callAsFunction(_ input: Tensor<Float>) -> Tensor<Float> {
        ...
        //var out2 = out1.sequenced(through: avgPool, outputClassifier)
        //out2 = out2.reshaped(to: [out2.shape[0], 1, 1, out2.shape[1]])
        //return out2.sequenced(through: outputAvgPool, output)
        return out1.sequenced(through: avgPool, outputClassifier)
    }
}


extension PostureNetV1: SummaryLayer {
    public func summary(scope: String) {
        //print("--PostureNetV1:summary-- \(scope)")
        
        print("PostureNetV1:                                                          \(scope)")
        print("-------------")
...
extension PostureNetV1: ExportableLayer {
    ...
    public func save_weight(scope: String, tensors: inout [String: Tensor<Float>]) {
        print("--PostureNetV1:save_weight-- \(scope)")
